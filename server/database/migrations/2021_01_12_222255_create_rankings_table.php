...
class CreateRankingsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('rankings', function (Blueprint $table) {
            ...
            $table->string('name_en', 100)->commnet('選手名');
            $table->string('name_jp', 100)->commnet('選手名');
            $table->integer('age')->length(3)->comment('年齢');
            $table->string('country', 100)->comment('所属国');
            ...
            $table->date('ymd')->comment('ランキングが更新された日');
            $table->timestamps();

            $table->unique(['name_en','ymd']);
        });
    }
